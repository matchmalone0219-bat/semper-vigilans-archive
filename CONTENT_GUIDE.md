...
12. **分析性评述收敛于按语，且统一显式标注【本站整理】**：
    - 所有关于剧情走向、伏笔联想、风格定调等二级推论或背景补充，必须全部放在 `note` / 按语字段中，绝不可混入直接引语本身。
    - 凡包含本站考据、编辑整理的按语，统一以 `【本站整理】` 或 `[ARCHIVE CURATOR NOTE / 档案注记]` 开头，严格区分一手客观证据与档案员整理意见。
13. **史料原版与本站扩展彻底解耦（数据与标签两立）**：
    - 官方历史原版（`historical`）：必须对应 100% 官方当年真实发布的一手物证与文本（如 GCPD 嫌犯通缉素描三连图、官方密码本、GOOD BYE <?>）。严禁把非原版素材挂上“官方原版”标签。
...
14. **全站事实数据与公映档期强一致性**：
    - 影片核心事实（如《新蝙蝠侠 2》正式北美公映档期 **2028 年 2 月 18 日**）必须在全站所有数据表、时间线、彩蛋与终端说明中保持 100% 绝对一致，杜绝任何旧草案档期造成的内部矛盾。
