...
12. **访谈及一手发言的分析性评述收敛于按语，并显式标注【本站整理】**：
    - 本条适用于访谈、直接引语及其他以一手发言为主体的条目。剧情走向、伏笔联想、风格定调、背景解释等二级推论不得混入 `quoteZh` / 直接引语本身，应放入 `note` / 按语字段。
    - 凡包含本站考据、编辑整理或推论的按语，统一以 `【本站整理】` 或 `[ARCHIVE CURATOR NOTE / 档案注记]` 开头，严格区分一手客观证据与档案员整理意见。
    - 剧情线索、案件、人物关系、艺术溯源等本身以分析为主体的栏目，可在其既有结构内撰写分析正文；无需强行迁移到不存在的 `note` 字段，但仍须区分事实、线索、传闻与本站推论。
