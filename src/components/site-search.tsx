...
export function SiteSearch() {
  ...
  const dialog =
    open && typeof document !== "undefined"
      ? createPortal(
          <div
            className="fixed inset-0 z-[90] bg-bg/90 p-4 backdrop-blur-sm sm:p-8"
            role="dialog"
            aria-modal="true"
            aria-label="全站搜索"
            onMouseDown={(event) => {
          ...
          >
            <div className="mx-auto mt-12 max-w-2xl border border-fg/15 bg-surface shadow-2xl sm:mt-20">
              <div className="flex items-center gap-3 border-b border-fg/10 px-4">
                <Search className="size-5 shrink-0 text-blood" />
                <input
                  ref={inputRef}
                  value={query}
  ...
  return (
    <>
      <button
        ...
        className="relative z-[91] inline-flex size-10 items-center justify-center text-muted hover:text-fg md:h-auto md:w-auto md:gap-2 md:border md:border-fg/15 md:px-3 md:py-2"
        aria-label="搜索全站"
        aria-expanded={open}
      >
